...
@pytest.mark.asyncio
class TestDocTreeWidgetAsync:
    """Async tests for DocTreeWidget."""

    async def test_widget_mounts_and_shows_tree(self) -> None:
        """Test that the widget mounts and displays the tree."""
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async with app.run_test():
                ...
                # Should have a DocTreeWidget
                tree_widget = app.screen.query_one("#doc-tree", DocTreeWidget)
                assert tree_widget is not None

                # Tree should have contents
                children = list(tree_widget.root.children)
                assert len(children) == 2  # docs/ and README.md

    async def test_cursor_navigation(self) -> None:
        """Test cursor navigation with j/k keys."""
        from iterm_controller.app import ItermControllerApp
        from iterm_controller.models import Project
        from iterm_controller.screens.modes.docs_mode import DocsModeScreen
    ...
    async def test_refresh_action(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async with app.run_test() as pilot:
                ...
                # Initially empty
                assert len(list(tree_widget.root.children)) == 0
