...
class TestActiveWorkSessionLinking:
    ...
    def test_get_session_for_task_reverse_lookup(self) -> None:
        """Test getting session via task_id reverse lookup.

        When task has no session_id set, the widget should search
        sessions for one whose task_id matches the task.
        """
        session = make_session(session_id="sess-1")
        session.task_id = "1.1"  # Link via task_id field
        session.metadata["task_title"] = "Test Task"

        # Task has no session_id but should still find session via task_id
    ...
    def test_get_session_for_task_prefers_direct_link(self) -> None:
        ...
        If both task.session_id and a session's task_id match,
